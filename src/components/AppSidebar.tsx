...
import { Home, Calendar, User, Settings, X, Users, LogIn } from 'lucide-react';
...
export function AdminSidebar({ sidebarOpen, setSidebarOpen }: AppSidebarProps) {
  const [activeSection, setActiveSection] = React.useState("dashboard");

  const handleMenuClick = (itemId: string) => {
    setActiveSection(itemId);
    window.dispatchEvent(new CustomEvent('adminSectionChange', { detail: itemId }));
    if (window.innerWidth < 1024) {
      setSidebarOpen(false);
    }
  };
